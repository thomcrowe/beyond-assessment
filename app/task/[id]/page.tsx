...
export default function TaskPage() {
  ...
  function canComplete() {
    if (taskId === 1 || taskId === 2) return response.trim().length > 100
    return aiPrompt.trim().length > 50 && aiOutput.trim().length > 50 && aiInterpretation.trim().length > 50 && aiRecommendation.trim().length > 50
  }
...
function TaskInstructions({ taskId }: { taskId: number }) {
  if (taskId === 1) return (
    <div>
      ...
      <Callout>Do you see the right problem? Do you prioritize ruthlessly, or try to fix everything at once? Does your intervention logic connect cause to outcome? Back everything with data, not instinct.</Callout>
    </div>
  )
  ...
  if (taskId === 2) return (
    <div>
      ...
      <Callout>Can you read metrics and copy together and draw the right conclusion? Do you understand what a host needs at this stage of their journey?</Callout>
    </div>
  )
  ...
  return (
    <div>
      ...
      <Callout>How you direct AI matters as much as what it gives you back. Show a strong, specific prompt. Apply judgment to the output. The final recommendation should be grounded in data from this brief -- not generated by the model.</Callout>
    </div>
  )
}
